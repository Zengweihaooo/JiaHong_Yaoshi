      ...
      <section class="filter-card" aria-label="问诊记录筛选">
        ...
        <div class="batch-bar">
          <div class="batch-actions">
            ...
            <button class="batch-action" :class="{ active: selected.length }" type="button" @click="batchAction('批量下载')">批量下载</button>
            <button class="batch-action" :class="{ active: selected.length }" type="button" @click="batchAction('批量打印')">批量打印</button>
            <button class="batch-action wide" :class="{ active: selected.length }" type="button" @click="batchAction('批量下载药品实名问诊')">批量下载药品实名问诊</button>
          </div>
...
const datePickerMode = computed(() => {
  if (props.datePickerVariant === "a") return "range";
  if (props.datePickerVariant === "b") return "split";
  return "range";
});
const dateRange = ref([]);
const dateStart = ref("");
...
watch(datePickerMode, (mode, previousMode) => {
  if (mode === previousMode) return;
  if (mode === "range") {
    const nextRange = [dateStart.value, dateEnd.value].filter(Boolean);
    dateRange.value = nextRange.length ? nextRange : [];
    return;
  }
...
function toggleMore(id, event) { pageSizeOpen.value = false; exportOpen.value = false; historyTipOpen.value = false; openSelectKey.value = ""; if (moreId.value === id) { moreId.value = null; return; } const cell = event.currentTarget.closest("td"); const button = event.currentTarget.getBoundingClientRect(); const anchor = cell?.getBoundingClientRect() || button; moreMenuStyle.value = { left: `${Math.round(button.left)}px`, top: `${Math.round(anchor.top + 48)}px` }; moreId.value = id; }
...
.table-scroll th:nth-child(7), .table-scroll th:nth-child(8), .table-scroll th:nth-child(9), .table-scroll th:nth-child(10), .table-scroll td:nth-child(7), .table-scroll td:nth-child(8), .table-scroll td:nth-child(9), .table-scroll td:nth-child(10) { text-align: center; }
...
.type-tag { display: inline-flex; align-items: center; justify-content: center; height: 32px; padding: 0 12px; border-radius: 3px; }.type-tag.text { color: #009080; background: #d1f0ec; }.type-tag.video { color: #5c82e6; background: #edf2ff; }
.consult-status { display: inline-flex; align-items: center; justify-content: center; gap: 8px; }.consult-status i { width: 12px; height: 12px; border-radius: 50%; }.consult-status .green { background: #2dcc70; }.consult-status .gray { background: #9aa7ba; }
